    ...
    >
      ...
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        ...
        {!showConfirm ? (
          ...
          <div className="flex items-center gap-2">
            <span className="font-pixel text-[0.45rem] sm:text-[0.5rem] text-danger tracking-wider3">SURE?</span>
            <button
              onClick={() => { onClear(); setShowConfirm(false); }}
              className="font-pixel text-[0.45rem] sm:text-[0.5rem] text-bg-base bg-danger border border-danger px-2 py-0.5 cursor-pointer hover:opacity-80 transition-opacity"
            >
              YES
            </button>
            <button
              onClick={() => setShowConfirm(false)}
              className="font-pixel text-[0.45rem] sm:text-[0.5rem] text-text-muted border border-border-strong px-2 py-0.5 bg-bg-surface cursor-pointer hover:border-text-sub transition-colors"
            >
              NO
            </button>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {entry.wpm}
                      {isBest && (
                        <span className="font-pixel text-[0.4rem] text-amber ml-1">★</span>
                      )}
                    </td>
                    {/* ACC */}
                    <td className="font-mono text-xs sm:text-sm text-text-sub px-3 py-2">
                      {entry.accuracy}%
                    </td>
                    {/* CORRECT */}
                    <td className="font-mono text-xs sm:text-sm text-accent px-3 py-2">
                      {entry.correct}
                    </td>
                    {/* ERRORS */}
                    <td
